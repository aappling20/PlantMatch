...
const injectStyles = () => {
  ...
  style.innerHTML = `
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    :root{--cream:#f4efe6;--dark:#1c2b1d;--moss:#3b5c3d;--sage:#6e9470;--leaf:#a8c5a0;--mist:#e4ede1;--gold:#b8924a;--bark:#4a3728;--white:#fdfaf5;}
    ...
    .navbar-logo{font-family:'Cormorant Garamond',serif;font-size:1.55rem;font-weight:700;letter-spacing:.03em;color:var(--moss);cursor:pointer}
    .navbar-logo .accent{color:var(--gold);font-style:italic}
    .hero{width:100%;min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;position:relative;overflow:hidden;background:linear-gradient(155deg,#e6efe2 0%,#f4efe6 45%,#ede8db 100%);padding-top:62px;}
    .blob{position:absolute;border-radius:50%;filter:blur(70px);opacity:.25;pointer-events:none;z-index:0}
    .b1{width:500px;height:500px;background:#a8c5a0;top:-100px;left:-100px}
    .b2{width:400px;height:400px;background:#c9a84c44;bottom:-80px;right:-80px}
    .b3{width:260px;height:260px;background:#6e9470;top:40%;left:58%}
    .hero-content{position:relative;z-index:1;text-align:center;padding:2rem 1.5rem;width:100%;max-width:680px;margin:0 auto;animation:fadeUp .85s ease both;}
    .eyebrow{font-size:.72rem;letter-spacing:.22em;text-transform:uppercase;color:var(--sage);margin-bottom:1.1rem;font-weight:500;display:block}
    .hero-title{font-size:clamp(3.2rem,8vw,6rem);font-weight:700;line-height:1.05;color:var(--dark)}
    .hero-title em{color:var(--moss);font-style:italic}
    .hero-sub{font-size:1.05rem;color:var(--bark);line-height:1.75;margin:1.5rem auto 2.5rem;font-weight:300;max-width:460px}
    .cta-row{display:flex;gap:1rem;justify-content:center;flex-wrap:wrap}
    .hero-scroll{position:absolute;bottom:1.8rem;left:50%;font-size:.68rem;letter-spacing:.16em;text-transform:uppercase;color:var(--sage);display:flex;flex-direction:column;align-items:center;gap:.45rem;animation:bounce 2.5s ease-in-out infinite;z-index:1}
    .about{width:100%;background:var(--dark);color:var(--mist);padding:5rem 2rem;text-align:center}
    ...
    .quiz-page{width:100%;min-height:100vh;padding:80px 1.5rem 3rem;display:flex;flex-direction:column;align-items:center;justify-content:center;background:linear-gradient(170deg,#edf3e9 0%,#f4efe6 100%)}
    ...
    .results-page{width:100%;min-height:100vh;padding:90px 2rem 4rem;background:linear-gradient(160deg,#f0f5ed 0%,#f4efe6 100%)}
    ...
    .card-img{width:100%;height:195px;object-fit:cover;display:block;background:var(--mist)}
    ...
    .icon-label{font-weight:500;color:var(--sage)}
    ...
    .popup-hero{width:100%;height:260px;position:relative;border-radius:28px 28px 0 0;overflow:hidden}
    .popup-hero img{width:100%;height:100%;object-fit:cover;display:block}
    .close-btn{position:absolute;top:1rem;right:1rem;width:36px;height:36px;border-radius:50%;background:rgba(255,255,255,.88);border:none;cursor:pointer;font-size:1.1rem;display:flex;align-items:center;justify-content:center;transition:background .2s,transform .2s;z-index:2}
    .close-btn:hover{background:white;transform:scale(1.1)}
    .match-badge{position:absolute;bottom:1rem;left:1.5rem;background:var(--moss);color:white;font-family:'Jost',sans-serif;font-size:.78rem;font-weight:600;letter-spacing:.06em;padding:.38rem 1rem;border-radius:50px;z-index:2}
    ...
    .stat{background:var(--mist);border-radius:14px;padding:1rem 1.2rem}
    .stat-label{font-size:.65rem;text-transform:uppercase;letter-spacing:.1em;color:var(--sage);margin-bottom:.25rem}
...
const QUESTIONS = [
  { id:"skill", question:"What's your experience with plants?", hint:"Be honest — there's a perfect plant for every level!",
    options:[{value:"beginner",label:"Beginner — I've killed a cactus"},{value:"intermediate",label:"Intermediate — I keep a few alive"},{value:"advanced",label:"Advanced — practically a botanist"}]},
  { id:"light", question:"How much natural light does your space get?", hint:"Think about the room where the plant will actually live",
    options:[{value:"low",label:"Low light — north-facing or shady"},{value:"medium",label:"Moderate — bright but indirect"},{value:"high",label:"Bright & sunny — south or west windows"}]},
  { id:"water", question:"How often are you willing to water?", hint:"Pick your real habit, not your aspirational one",
    options:[{value:"rarely",label:"Once every 2–3 weeks or less"},{value:"weekly",label:"About once a week"},{value:"frequent",label:"Multiple times a week — I love it"}]},
  { id:"time", question:"How much time do you want to spend on care?", hint:"Includes pruning, fertilising, repotting",
    options:[{value:"minimal",label:"Minimal — set and forget"},{value:"moderate",label:"A few minutes here and there"},{value:"lots",label:"I love tending my plants"}]},
  { id:"location", question:"Where will your plant live?", hint:"This affects light, temperature, and humidity needs",
    options:[{value:"indoor",label:"Indoors — apartment or house"},{value:"outdoor",label:"Outdoors — garden or balcony"},{value:"both",label:"Both — I'll move it around"}]},
  { id:"pets", question:"Do you have pets or young children?", hint:"Some plants are toxic if chewed",
    options:[{value:"yes",label:"Yes — safety is a priority"},{value:"no",label:"No — no worries there"}]},
  { id:"size", question:"What size plant are you looking for?", hint:"Consider the space you have available",
    options:[{value:"small",label:"Small — desk or shelf sized"},{value:"medium",label:"Medium — floor plant, knee height"},{value:"large",label:"Large — statement piece, fills a corner"}]},
];

const PLANTS = [
  { id:1,name:"Snake Plant",latin:"Sansevieria trifasciata",photo:"/PlantMatch/images/Snake+Plant.webp",
    tags:["Low maintenance","Air purifying","Drought tolerant"],skill:["beginner","intermediate","advanced"],light:["low","medium","high"],water:["rarely","weekly"],time:["minimal","moderate"],location:["indoor","both"],pets:["no"],size:["medium","large"],
  ...
  { id:2,name:"Pothos",latin:"Epipremnum aureum",photo:"/PlantMatch/images/marble-queen-pothos-28748821463082_x560.progressive.webp",
    tags:["Trailing vines","Fast growing","Very forgiving"],skill:["beginner","intermediate"],light:["low","medium"],water:["weekly"],time:["minimal","moderate"],location:["indoor","both"],pets:["no"],size:["small","medium"],
  ...
  { id:3,name:"Monstera",latin:"Monstera deliciosa",photo:"/PlantMatch/images/bloomscape_monstera_alt_stone_580x696_crop_center@2x.webp",
    tags:["Statement plant","Iconic leaves","Tropical"],skill:["intermediate","advanced"],light:["medium","high"],water:["weekly"],time:["moderate"],location:["indoor","both"],pets:["no"],size:["large"],
  ...
  { id:4,name:"ZZ Plant",latin:"Zamioculcas zamiifolia",photo:"/PlantMatch/images/Plants-265_zz.webp",
    tags:["Near-indestructible","Low light","Glossy leaves"],skill:["beginner","intermediate"],light:["low","medium"],water:["rarely"],time:["minimal"],location:["indoor"],pets:["no"],size:["medium"],
  ...
  { id:5,name:"Spider Plant",latin:"Chlorophytum comosum",photo:"/PlantMatch/images/Spider-Plant-Main-1229x1536.webp",
    tags:["Pet-safe","Air purifying","Easy care"],skill:["beginner","intermediate"],light:["low","medium","high"],water:["weekly"],time:["minimal","moderate"],location:["indoor","both"],pets:["yes"],size:["small","medium"],
  ...
  { id:6,name:"Boston Fern",latin:"Nephrolepis exaltata",photo:"/PlantMatch/images/Boston-Fern-3G-800x1000.jpg",
    tags:["High humidity","Lush foliage","Pet-safe"],skill:["intermediate","advanced"],light:["medium"],water:["frequent"],time:["lots","moderate"],location:["indoor","both"],pets:["yes"],size:["medium"],
  ...
  { id:7,name:"Succulents",latin:"Various genera",photo:"/PlantMatch/images/succulent-garden.jpg.webp",
    tags:["Drought tolerant","Colourful","Compact"],skill:["beginner","intermediate"],light:["high","medium"],water:["rarely"],time:["minimal"],location:["indoor","outdoor","both"],pets:["no"],size:["small"],
  ...
  { id:8,name:"Peace Lily",latin:"Spathiphyllum wallisii",photo:"/PlantMatch/images/spathiphyllum_peace_lily_2-min-min_f8a2fb41-f073-4a6c-9c47-33b8a179e2e2.webp",
    tags:["Flowering","Air purifying","Low light"],skill:["beginner","intermediate"],light:["low","medium"],water:["weekly"],time:["moderate"],location:["indoor"],pets:["no"],size:["medium"],
  ...
  { id:9,name:"Rubber Plant",latin:"Ficus elastica",photo:"/PlantMatch/images/rubbertree-1.jpg",
    tags:["Bold statement","Fast growing","Structural"],skill:["intermediate","advanced"],light:["medium","high"],water:["weekly"],time:["moderate"],location:["indoor","both"],pets:["no"],size:["large"],
  ...
  { id:10,name:"Aloe Vera",latin:"Aloe barbadensis miller",photo:"/PlantMatch/images/Aloe-Vera.jpg",
    tags:["Medicinal","Drought tolerant","Outdoor-friendly"],skill:["beginner","intermediate"],light:["high","medium"],water:["rarely"],time:["minimal"],location:["indoor","outdoor","both"],pets:["no"],size:["small","medium"],
...
function Navbar({ page, onHome }) {
  return (
    <nav className="navbar">
      <div className="navbar-logo" onClick={onHome}>Plant<span className="accent">Match</span></div>
      {page !== "home" && <button className="pm-btn btn-ghost" onClick={onHome}>← Home</button>}
    </nav>
  );
}
    ...
    <div>
      <section className="hero">
        ...
        <div className="hero-content">
          <span className="eyebrow">Your personal plant guide</span>
          <h1 className="hero-title">Find Your<br/><em>Perfect Plant</em></h1>
          <p className="hero-sub">Answer 7 quick questions about your lifestyle, space, and experience. We will match you with plants that will actually thrive in your home.</p>
      ...
      <section className="about">
        ...
        <div className="about-cards">
          {["Take the Quiz","Get Matched","Learn & Grow"].map(label=>(
            <div key={label} className="about-card"><div className="about-card-label">{label}</div></div>
          ))}
        </div>
      </section>
    </div>
...
function QuizPage({ onComplete }) {
  ...
  const next = () => { if (!sel) return; if (step < QUESTIONS.length-1){setStep(s=>s+1);setKey(k=>k+1);}else onComplete(answers); };
    ...
    <div className="quiz-page">
      <div className="quiz-shell" key={key}>
        ...
        <div className="options">
          {q.options.map((o,i)=>(
            <div key={o.value} className={`option${sel===o.value?" sel":""}`} style={{animationDelay:`${i*0.07}s`}} onClick={()=>setAnswers(p=>({...p,[q.id]:o.value}))}>
        ...
        <div className="quiz-nav">
          ...
          <button className="pm-btn btn-primary" onClick={next} style={{opacity:sel?1:0.4,cursor:sel?"pointer":"not-allowed",padding:".75rem 2rem"}}>
      ...
      <div className="popup" onClick={e=>e.stopPropagation()}>
        ...
        <div className="popup-body">
          ...
          <div className="stats-grid">
            {[{label:"Watering",val:plant.stats.water},{label:"Light",val:plant.stats.light},{label:"Humidity",val:plant.stats.humidity},{label:"Temperature",val:plant.stats.temp}].map(s=>(
              <div key={s.label} className="stat"><div className="stat-label">{s.label}</div><div className="stat-val">{s.val}</div></div>
            ))}
          </div>
          <h3 className="section-title">Care Guide</h3>
          <div className="care-steps">
            {plant.care.map((c,i)=>(
              <div key={i} className="care-step"><div className="care-num">{i+1}</div><p className="care-text">{c}</p></div>
            ))}
          </div>
          <div className="popup-tags">{plant.tags.map(t=><span key={t} className="tag">{t}</span>)}</div>
        </div>
      </div>
    </div>
  );
...
function ResultsPage({ answers, onRetake }) {
  ...
  const ranked = PLANTS.map(p=>({...p,match:scorePlant(p,answers)})).sort((a,b)=>b.match-a.match);
  ...
  return (
    <div className="results-page">
      <div className="results-header">
        ...
        <p className="results-sub">Ranked by compatibility — your top match is <strong>{ranked[0]?.name}</strong> at {ranked[0]?.match}%</p>
      ...
      <div className="grid">
        {filtered.map((plant,i)=>(
          <div key={plant.id} className="plant-card" style={{animationDelay:`${i*0.06}s`}} onClick={()=>setPopup(plant)}>
            <img className="card-img" src={plant.photo} alt={plant.name}/>
            ...
            <div className="card-body">
              ...
              <div className="card-icons">
                <div className="icon-item"><span className="icon-label">Water:</span>&nbsp;{plant.stats.water.split(" ").slice(0,3).join(" ")}</div>
                <div className="icon-item"><span className="icon-label">Light:</span>&nbsp;{plant.stats.light.split(" ").slice(0,2).join(" ")}</div>
              </div>
            </div>
          </div>
        ))}
